...
async def get_available_formats(link: str):
    ...
    if proc.returncode != 0:
        print(f"Error fetching formats: {stderr.decode()}")
        return None
...
class VoiceChatMod(loader.Module):
    ...
    async def client_ready(self, client, db):
        ...
        self.musicdl = await self.import_lib(
            "https://raw.githubusercontent.com/pussyklller/musicdl/refs/heads/main/musicdl.py",
    ...
    async def stream_ended(self, client: PyTgCalls, update: types.Update):
        ...
        if self._queue[chat_id][0]["audio"]:
            ...
            if self._queue[chat_id][0]["youtube"]:
                await self.play_video_ytt(chat_id, self._queue[chat_id][0]["data"])
            else:
    ...
    async def play_video_ytt(self, chat_id: int, link: str):
        formats = await get_available_formats(link)
        if formats is None:
            return

        best_video_format = None
        if "136" in formats:
            best_video_format = "136+140"  # 720p
        elif "137" in formats:
            best_video_format = "137+140"  # 1080p
        elif "135" in formats:
            best_video_format = "135+140"  # 480p
        else:
    ...
    @loader.command(ru_doc="<ответить на видео или ссылку> - Добавить видео в очередь голосового чата", ua_doc="<відповісти на відео або посилання> - додайте відео до черги голосового чату")
    ...
    @loader.command(ru_doc="<ответить на видео или ссылку YT> - Добавить видео в очередь голосового чата", ua_doc="<відповісти на відео або посилання YT> - Додати відео до черги голосового чату")
    async def yt(self, message: Message):
        """<reply to video or yt link> - Add video to chat's voicechat queue"""
        reply = await message.get_reply_message()
        link = utils.get_args_raw(message)
        if (not reply or not reply.media) and not link:
        ...
        try:
            if not any(i["playing"] for i in self._queue[chat_id]):
                self._queue[chat_id][-1]["playing"] = True
                if self._queue[chat_id][-1]["youtube"]:
                    await self.play_video_ytt(chat_id, raw_data)
                else:
                    await self.play_video(chat_id, raw_data)
        except BadRequestError as e:
            if "TOPIC_CLOSED" in str(e):
    ...
    @loader.command(ru_doc="Пропускает текущую музыку в очереди", ua_doc="Пропускає поточну пісню в черзі")
    ...
    @loader.command(ru_doc="Сохранить очередь", ua_doc="Зберегти чергу")
    ...
    @loader.command(ru_doc="Включите функцию «повторять эту очередь вечно»", ua_doc="Перемкніть «повторювати цю чергу назавжди».")
    ...
    @loader.command(ru_doc="Удалить песню из очереди", ua_doc="Видалити пісню з черги")
    ...
    @loader.command(ua_doc="Повторно приєднайтеся до дзвінка, якщо ви випадково приєдналися до нього", ru_doc="Повторно присоединитесь к звонку, если вы случайно к нему присоединились.")
    async def rst(self, message: Message):
        ...
        if self._queue[chat_id][0]["audio"]:
            ...
            if self._queue[chat_id][-1]["youtube"]:
                await self.play_video_ytt(chat_id, self._queue[chat_id][-1]["data"])
            else:
                await self.play_video(chat_id, self._queue[chat_id][-1]["data"])
        await message.delete()

    @loader.command(ua_doc="Відновити поточну чергу чату", ru_doc="Возобновить очередь текущего чата")
    ...
    @loader.command(ua_doc="Призупинити поточну чергу чату", ru_doc="Приостановить очередь текущего чата")
    ...
    @loader.command(ua_doc="Зупинити поточну чергу чату", ru_doc="Остановить очередь текущего чата")
    async def stop(self, message: Message):
        """Stop current chat's queue"""
        await self._inline__stop(message, utils.get_chat_id(message))

    @loader.command(ru_doc="Показать панель управления", ua_doc="Показати панель керування")

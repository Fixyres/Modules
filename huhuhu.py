...
class huhuhu(loader.Module):
    """
    Модуль созданныйllkdkkddk
    """
    ...
    async def p(self, message: Message):
        ...
        if not args:
            if message.is_reply:
                ...
                await message.reply("Пожалуйста, ответьте на сообщение пользователя или укажите ID/username.")
                return
        else:
            user = args[0]
            if user.isdigit():
                ...
                try:
                    ...
                    await message.reply(f"Не удалось получить информацию о пользователе: {str(e)}")
                    return

        async with message.client.conversation(self._bot) as conv:
            try:
                await conv.send_message(f"Профиль {user_id}")

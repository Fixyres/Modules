...
class AutoTranslatorMod(loader.Module):
    """Automatically translates your messages using Google Translate"""
    ...
    @loader.command(
        ...
        uk_doc="Показати список доступних мов"
    )
    async def tr_langs(self, message):
        """Show list of available languages"""
        langs_text = "\n".join([f"<code>{code}</code> - {name}" for code, name in self.SUPPORTED_LANGS.items()])
    ...
    @loader.command(
        ...
        uk_doc="[мова] - Увімкнути/вимкнути автопереклад у чаті"
    )
    async def autotr(self, message):
        ...
        # Если уже есть активный перевод, выключаем его
        if chat_id in self._chats:
            old_lang = self._chats[chat_id]
            del self._chats[chat_id]
            self.db.set("AutoTranslator", "chats", self._chats)
            await utils.answer(message, self.strings("disabled").format(f"{old_lang} ({self.SUPPORTED_LANGS[old_lang]})"))

            # Если не указан новый язык, завершаем
            if not args:
                return
            # Иначе делаем небольшую паузу перед включением нового
    ...
    async def watcher(self, message):
        ...
        if not getattr(message, "text", False):
            return

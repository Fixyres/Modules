...
# Ваша предыдущая часть кода...

class GIBFGBMod(loader.Module):
    strings = {"name": "GIBFGB"}
    
    _bot = "@bfgbunker_bot"    
    chat_id = -4536633154  # ID основного чата
    ...
    async def atakcmd(self, message: Message):
        ...
        await self.client.send_message(self.chat_id, order_message)
    ...
    async def g5cmd(self, message: Message):
        ...
        result_message = "📊 Статистика гильдии за текущую неделю:\n\n"
        result_message += "\n".join(self.result_list)
        result_message += f"\n\n<b>🗒Всего бутылок:</b> {self.total_bottles_str} 🍾"  
    ...
    async def five_percent(self, call: InlineCall):
        result_message = "5% в понедельник:\n\n"
        result_message += "\n".join(self.five_percent_bonus_list)
        result_message += f"\n\n<b>🗒Всего бутылок в 5% —</b> {self.total_five_percent_bonus_str} 🍾" 
    ...
    async def monday(self, call: InlineCall):
        result_message = "Понедельник:\n\n"
        result_message += "\n".join(self.monday_bottles_list)
        result_message += f"\n\n<b>🗒Всего бутылок в понедельник —</b> {self.total_monday_bottles_str} 🍾" 
        await call.edit(result_message, reply_markup=[
            [
                {
                    "text": "Назад",

...
class AutoPremium(loader.Module):
    ...
    def __init__(self):
        self.config = loader.ModuleConfig(
            ...
            loader.ConfigValue(
                "folder",
                True,
                "добавляет папку под названием AutoPremium, в которую добавляются все каналы на которые подписался модуль",
                validator=loader.validators.Boolean()
            )
        )
        
    async def client_ready(self, client, db):
        self.folder_name = "AutoPremium"
        self.client = client
    ...
    async def create_or_get_folder(self):
        """проверка на наличие папки AutoPremium и создание новой при необходимости"""
